...
function CreateUnitsForPlayer0 takes nothing returns nothing
    ...
    set u=CreateUnit(p, 'e00B', - 2444.4, 4561.9, 66.689)
endfunction
...
function DisplayMessageToPlayer takes string msg,integer playerid returns nothing
 local force onePlayerForce= GetForceOfPlayer(ConvertedPlayer(playerid))
	call DisplayTextToForce(onePlayerForce, msg)
	call DestroyForce(onePlayerForce)
	set onePlayerForce=null
endfunction

function DisplayMessageToTriggeringPlayer takes string msg returns nothing
 local force triggerplayer= GetForceOfPlayer(GetTriggerPlayer())
	call DisplayTextToForce(triggerplayer, msg)
	call DestroyForce(triggerplayer)
...
function InitializeVotes takes nothing returns nothing
 local location loc= Location(- 1900, 2100)
	...
	set udg_reals01[2]=150.00
	set udg_reals01[3]=200.00
	set udg_reals01[4]=250.00
...
function CloseSpawn takes string name,integer playerid returns nothing
	if ( udg_integer14 == 0 ) then
		...
		call DisplayMessageToTriggeringPlayer("There's already a poll going!")
	endif
endfunction
 ...
 local integer playerid= GetPlayerIdFromColorName(colorname)
	if ( playerid > 0 ) then
		if ( GetPlayerSlotState(ConvertedPlayer(playerid)) == PLAYER_SLOT_STATE_PLAYING ) then
			call DisplayMessageToTriggeringPlayer(GetPlayerName(ConvertedPlayer(playerid)) + " is playing, you can't close that spawn!")
		else
			if ( udg_IsSpawnOpen[playerid] == 0 ) then
				call DisplayMessageToTriggeringPlayer("That spawn is already closed!")
			else
				call CloseSpawn(colorname , playerid)
			endif
		endif
	else
		call DisplayMessageToTriggeringPlayer("Please enter a valid color!")
	endif
endfunction
...
function OpenSpawnCommand takes nothing returns nothing
 ...
 local integer playerid= GetPlayerIdFromColorName(colorname)
	if ( playerid > 0 ) then
		if ( GetPlayerSlotState(ConvertedPlayer(playerid)) == PLAYER_SLOT_STATE_PLAYING ) then
			call DisplayMessageToTriggeringPlayer(GetPlayerName(ConvertedPlayer(playerid)) + " is playing, you can't open that spawn!")
		else
			if ( udg_IsSpawnOpen[playerid] == 1 ) then
				call DisplayMessageToTriggeringPlayer("That spawn is already open!")
			else
 ...
 local integer playerid= GetPlayerIdFromColorName(colorname)
	if ( playerid > 0 ) then
		if ( playerid != GetConvertedPlayerId(GetTriggerPlayer()) ) then
			...
			call DisplayMessageToTriggeringPlayer(udg_PlayerColorCodes[playerid] + GetPlayerName(ConvertedPlayer(playerid)) + "|r is now |cFF00FF00allowed|r to build in your spawn!")
		else
			call DisplayMessageToTriggeringPlayer("You are already allowed to to build in this spawn")
		endif
	else
		call DisplayMessageToTriggeringPlayer("Please enter a valid color!")
	endif
 ...
 local integer playerid= GetPlayerIdFromColorName(colorname)
	if ( playerid > 0 ) then
		if ( playerid != GetConvertedPlayerId(GetTriggerPlayer()) ) then
			...
			call DisplayMessageToTriggeringPlayer(udg_PlayerColorCodes[playerid] + GetPlayerName(ConvertedPlayer(playerid)) + "|r is now |cFFFF0000denied|r access to your spawn!")
			call ChangeOwnershipOfAllDeniedAccessTowers(GetConvertedPlayerId(GetTriggerPlayer()))
		else
			call DisplayMessageToTriggeringPlayer("You can't deny yourself access!")
		endif
	else
		call DisplayMessageToTriggeringPlayer("Please enter a valid color!")
	endif
...
function CheckSelected2 takes nothing returns nothing
	call DisplayMessageToTriggeringPlayer("Selected unit has(" + I2S(LoadIntegerBJ(GetHandleIdBJ(GetEnumUnit()), 0, udg_UnitToCheckpointTable)) + ")")
endfunction
...
function PlayerChatCommandActions takes nothing returns nothing
    ...
    elseif ( GetEventPlayerChatString() == "-read" ) then
		call DisplayTextToForce(GetPlayersAll(), "GOT" + (HCL___command)) // INLINED!!
    elseif ( GetEventPlayerChatString() == "-setkills" ) then
		call MMD___update_value(("KILLS" ) , ( GetTriggerPlayer() ) , MMD___ops[( MMD_OP_ADD )] , I2S(( 100)) , MMD_TYPE_INT) // INLINED!!
	elseif ( GetEventPlayerChatString() == "-boss" ) then
		call DisplayMessageToTriggeringPlayer("|cFF3737F2Boss:|r 9 / 14 / 19 / 24 / 29 / 31")
	elseif ( GetEventPlayerChatString() == "-champ" ) then
		call DisplayMessageToTriggeringPlayer("|cFFF2A137Champion:|r 35 / 36")
	elseif ( GetEventPlayerChatString() == "-champion" ) then
		call DisplayMessageToTriggeringPlayer("|cFFF2A137Champion:|r 35 / 36")
	elseif ( GetEventPlayerChatString() == "-light" ) then
		call DisplayMessageToTriggeringPlayer("|cFF6d7c86Light:|r 4 / 8 / 11 / 16 / 19 / 23 / 27 / 32")
	elseif ( GetEventPlayerChatString() == "-medium" ) then
		call DisplayMessageToTriggeringPlayer("|cFF416073Medium:|r 3 / 7 / 12 / 17 / 24 / 28 / 33")
	elseif ( GetEventPlayerChatString() == "-heavy" ) then
		call DisplayMessageToTriggeringPlayer("|cFF154360Heavy:|r 2 / 5 / 13 / 15 / 20 / 25 / 30 / 32 / 35")
	elseif ( GetEventPlayerChatString() == "-fortified" ) then
		call DisplayMessageToTriggeringPlayer("|cFFCA8500Fortified:|r 10 / 18 / 22 / 26 / 31")
	elseif ( GetEventPlayerChatString() == "-hero" ) then
		call DisplayMessageToTriggeringPlayer("|cFF7525FFHero:|r 36")
	...
	elseif ( GetEventPlayerChatString() == "-repick" ) then
		if ( RepickConditions() ) then
			...
			call DisplayMessageToTriggeringPlayer("You can only repick before wave 1!")
		endif
	...
	elseif ( SubStringBJ(GetEventPlayerChatString(), 1, 8) == "-openall" ) then
		if ( GetPlayerName(Player(0)) == "WorldEdit" ) then
			call DisplayMessageToTriggeringPlayer("All spawns are now open!")
			call OpenAllSpawns()
		endif
	elseif ( SubStringBJ(GetEventPlayerChatString(), 1, 9) == "-closeall" ) then
		if ( GetPlayerName(Player(0)) == "WorldEdit" ) then
			call DisplayMessageToTriggeringPlayer("All spawns are now closed!")
			call CloseAllSpawns()
		endif
	...
	elseif ( SubStringBJ(GetEventPlayerChatString(), 1, 9) == "-allowall" ) then
		...
		call DisplayMessageToTriggeringPlayer("ALL players are now |cFF00FF00allowed|r to build in your spawn!")
	elseif ( SubStringBJ(GetEventPlayerChatString(), 1, 8) == "-denyall" ) then
		call DenyAllPlayers()
		call DisplayMessageToTriggeringPlayer("ALL players are now |cFFFF0000denied|r access to your spawn!")
	elseif ( SubStringBJ(GetEventPlayerChatString(), 1, 6) == "-zoom " ) then
		call ZoomPlayerCamera(SubStringBJ(GetEventPlayerChatString(), 7, StringLength(GetEventPlayerChatString())))
	elseif ( SubStringBJ(GetEventPlayerChatString(), 1, 5) == "-cam " ) then
		call ZoomPlayerCamera(SubStringBJ(GetEventPlayerChatString(), 6, StringLength(GetEventPlayerChatString())))
		...
		if ( udg_HasHybridRandomed[playerid] ) then
			...
			if ( not ( udg_HasHardcoreRandomed[playerid] ) ) then
				if ( udg_RepickCounter[playerid] == 0 ) then
					...
					call AdjustPlayerStateBJ(1, GetOwningPlayer(GetBuyingUnit()), PLAYER_STATE_RESOURCE_LUMBER)
					call DisplayMessageToTriggeringPlayer("You can't hardcore random after using normal random!")
				endif
			else
				call AdjustPlayerStateBJ(1, GetOwningPlayer(GetBuyingUnit()), PLAYER_STATE_RESOURCE_LUMBER)
			endif
...
call ExecuteFunc("jasshelper__initstructs11291984")
...
function jasshelper__initstructs11291984 takes nothing returns nothing
